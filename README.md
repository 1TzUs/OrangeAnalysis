...
三谋战报图像识别与数据分析工具。上传战报截图，自动识别阵营、武将、兵力、胜负与红度，生成胜率排行与战斗矩阵热力图，并支持本地/云端数据同步。
...
- **全屏表格查看**：胜率排行与对战热力图一键「展开」铺满窗口，全量展示、独立筛选、按胜率排序、两表互切，关闭或 Esc 退出。
- **深浅主题**：页头一键切换深 / 浅配色，选择自动记忆。
- **设置自动保存**：设置改动立即生效并提示「已保存」，无需手动保存。
- **数据管理**：本地数据增删、导入/导出 JSON、云端（JSONBin.io）下载/合并去重上传/清空。
- **洞察页**：独立「洞察」标签页——阵容近 N 天逐日走势柱状图（窗口可调）· 对阵克制与「最强对手 / 最弱对手」标注 · 一键导出 CSV / Canvas 海报。
